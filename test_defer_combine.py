...
from label_combiner import MAPOracleCombiner
...
def main():
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        combiner.fit(machine_output.cpu().detach().numpy(), np.array(expert_pred[i]), target.cpu().detach().numpy())
        ...
        label_combination = combiner.combine(machine_output.cpu().detach().numpy(), np.array(expert_pred[i]))
        for j in range(len(defer_output)):
            if defer_output[j]==0:
                ...
                defer += 1
                if label_combination[j]==target[j]:
                    corr+=1
